...
func (g *Generator) GenerateToolFiles(config *converter.MCPConfig) error {
	...
	for _, tool := range config.Tools {
		...
		outputFilePath := filepath.Join(g.outputDir+"/pkg/mcptools", outputFileName)
		...
		fmt.Fprintf(&toolBuf, "// Code generated by https://github.com/flowgent-labs/mcpfather\n\n")
		...
		if err != nil {
			dumpPath := filepath.Join(g.outputDir, "pkg", "mcptools", outputFileName+".bad")
		...
		err = writeFileContent(g.outputDir+"/pkg/mcptools", outputFileName, func() ([]byte, error) {
