...
export MCP__AUTH__FRONTEND__OIDC__AUDIENCE="jira-mcp"
./bin/jira-mcp -v 10 --transport http --port 8080
...
#{"resource":"jira-mcp","authorization_servers":["https://keycloak.example.com/realms/master"],"bearer_methods_supported":["header"]}
```

### Run in `CLI` mode (example)

```sh
./bin/jira-mcp -v 1 -t cli list
./bin/jira-mcp -v 1 -t cli <tool-name> [OPTIONS]
```
...
```json
{
  "mcp": {
    "jira-mcp": {
      "type": "local",
      "command": ["bash", "-c", "/path/to/jira-mcp"],
...
```sh
docker build -t ghcr.io/<YOUR_ORG>/jira-mcp:latest -f deploy/docker/Dockerfile .
docker push ghcr.io/<YOUR_ORG>/jira-mcp:latest
```
...
```sh
docker pull ghcr.io/<YOUR_ORG>/jira-mcp:latest
```

First generate a default config, then mount it when running:

```sh
  docker run --rm ghcr.io/<YOUR_ORG>/jira-mcp:latest --print-default-config > ~/.jira-mcp/config.yaml
```
...
```sh
  docker run -d --name jira-mcp \
    ...
    -v ~/.jira-mcp:/home/mcp/.jira-mcp \
    ghcr.io/<YOUR_ORG>/jira-mcp:latest \
...
# Call any tool (example: AcknowledgeErrors)
./mcpclient.sh call AcknowledgeErrors '{"<param>":"<value>"}'
...
```sh
  helm upgrade -i jira-mcp deploy/helm \
    --set image.repository=ghcr.io/<YOUR_ORG>/jira-mcp \
