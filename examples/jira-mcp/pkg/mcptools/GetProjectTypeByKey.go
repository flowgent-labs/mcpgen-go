// Code generated by https://github.com/flowgent-labs/mcpfather

package mcptools

import (
	"context"
	"github.com/mark3labs/mcp-go/mcp"
	"jira-mcp/pkg/helpers"
)

// Code generated by https://github.com/flowgent-labs/mcpfather

// Input Schema for the GetProjectTypeByKey tool
const GetProjectTypeByKeyInputSchema = "{\n  \"properties\": {\n    \"projectTypeKey\": {\n      \"description\": \"The key of the project type\",\n      \"type\": \"string\"\n    }\n  },\n  \"required\": [\n    \"projectTypeKey\"\n  ],\n  \"type\": \"object\"\n}"

// Response Template for the GetProjectTypeByKey tool (Status: 200, Content-Type: application/json)
const GetProjectTypeByKeyResponseTemplate_A = "# API Response Information\n\nBelow is the response template for this API endpoint.\n\nThe template shows a possible response, including its status code and content type, to help you understand and generate correct outputs.\n\n**Status Code:** 200\n\n**Content-Type:** application/json\n\n> Returns a representation of the project type with the given id\n\n## Response Structure\n\n- Structure (Type: object):\n  - **color** (Type: string):\n      - Example: '#FFFFFF'\n  - **descriptionI18nKey** (Type: string):\n      - Example: 'Project type for software projects'\n  - **formattedKey** (Type: string):\n      - Example: 'Software'\n  - **icon** (Type: string):\n      - Example: ''\n  - **key** (Type: string):\n      - Example: 'software'\n"

// NewGetProjectTypeByKeyMCPTool creates the MCP Tool instance for GetProjectTypeByKey
func NewGetProjectTypeByKeyMCPTool() mcp.Tool {
	return mcp.NewToolWithRawSchema(
		"GetProjectTypeByKey",
		"Get project type by key - Returns the project type with the given key. In case of anonymous checks if they can access at least one project.",
		[]byte(GetProjectTypeByKeyInputSchema),
	)
}

// GetProjectTypeByKeyHandler is the handler function for the GetProjectTypeByKey tool.
// It reads tool arguments, forwards the request to the upstream service, and returns the response.
func GetProjectTypeByKeyHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	upstream := mcputils.GetUpstreamEndpoint()

	args := request.GetArguments()
	if args == nil {
		args = make(map[string]interface{})
	}
	contentType := ""
	return mcputils.ForwardAndParseResponse(ctx, upstream, "GET", "/rest/api/2/project/type/{projectTypeKey}", args, []string{"projectTypeKey"}, contentType, "GetProjectTypeByKey")
}
