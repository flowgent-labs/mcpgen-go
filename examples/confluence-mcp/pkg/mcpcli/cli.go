...
import (
	...
	mcptools "confluence-mcp/pkg/mcptools"
	"github.com/mark3labs/mcp-go/mcp"

	"confluence-mcp/pkg/mcpvirtual/engine"
	"confluence-mcp/pkg/mcpvirtual/pipeline"
)

// virtualRegistry adapts native MCP tools for the virtual tool engine.
type virtualRegistry struct{}

func (r *virtualRegistry) CallTool(ctx context.Context, name string, args map[string]interface{}) (*pipeline.CallToolResult, error) {
...
// Respects the config.yaml tools.expose filter if present.
func ListTools() {
	cfg, _ := mcputils.LoadConfig("confluence-mcp")
...
// resolveEnabledTools determines which native tools are exposed based on config.
// Returns nil when all tools are exposed (no config or no expose section).
func resolveEnabledTools(cfg *mcputils.Config) map[string]bool {
	if cfg == nil || cfg.Tools.Expose == nil {
		return nil // nil = all tools exposed (backward compatible)
	}
	expose := cfg.Tools.Expose
	// check conflicts (only in the CLI, non-fatal: prints warning)
	for _, name := range expose.Includes {
		for _, x := range expose.Excludes {
			if name == x {
				fmt.Fprintf(os.Stderr, "Warning: tool %q appears in both includes and excludes\n", name)
			}
		}
	}
...
func Call(binName, toolName string, args []string) error {
	...
	// Try virtual tool
	vtTools := loadVirtualTools()
	if entry, ok := vtTools[toolName]; ok {
		return callVirtual(binName, toolName, args, entry)
	}
...
func callVirtual(binName, toolName string, args []string, entry pipeline.VirtualToolEntry) error {
	for _, arg := range args {
		if arg == "--help" || arg == "-h" {
