...
type ToolsExposeConfig struct {
	...
	// Excludes lists tools that are explicitly hidden from external agents.
	// If a tool appears in both Includes and Excludes, the server fails to start.
...
// VirtualConfigPath returns the path to the config file used by the virtual engine
