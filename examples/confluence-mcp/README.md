...
> An Enterprise-grade MCP server, specifically designed for AI to call Confluence Data Center APIs, generated from MCPFather — exposes **176 native tools**. Each API operation becomes an AI-callable tool with typed schemas and customized aggregate virtual tools, auth forwarding, and observability built-in, and even use as regular CLI.
...
```sh
./bin/confluence-mcp -v 1 -t cli list
./bin/confluence-mcp -v 1 -t cli <tool-name> [OPTIONS]
```
...
# Or manually:
docker build -t ghcr.io/your-org/confluence-mcp:latest -f deploy/docker/Dockerfile .

# Build and push to GitHub Container Registry (on push to main, CI does this automatically)
make build-image
docker push ghcr.io/your-org/confluence-mcp:latest
