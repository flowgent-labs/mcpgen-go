...
./bin/sonatypeiq-mcp --print-default-config

# Create and edit your config
mkdir -p ~/.sonatypeiq-mcp
./bin/sonatypeiq-mcp --print-default-config > ~/.sonatypeiq-mcp/config.yaml
```

Edit `~/.sonatypeiq-mcp/config.yaml` and configure `tools.expose` to control which tools are available:
...
export MCP_UPSTREAM_ENDPOINT=https://api.example.com
export MCP_UPSTREAM_TOKEN=your-token
./sonatypeiq-mcp --transport http --port 8080
```
