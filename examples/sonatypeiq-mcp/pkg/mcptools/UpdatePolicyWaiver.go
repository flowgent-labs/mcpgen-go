// Code generated by https://github.com/flowgent-labs/mcpfather

package mcptools

import (
	"context"
	"github.com/mark3labs/mcp-go/mcp"
	"sonatypeiq-mcp/pkg/helpers"
)

// Code generated by https://github.com/flowgent-labs/mcpfather

// Input Schema for the UpdatePolicyWaiver tool
const UpdatePolicyWaiverInputSchema = "{\n  \"properties\": {\n    \"body\": {\n      \"description\": \"Options for creating policy waivers\",\n      \"properties\": {\n        \"comment\": {\n          \"description\": \"Reason for waiving the violation(s). Must be non-blank.\",\n          \"example\": \"False positive - internal tool approved by security team\",\n          \"type\": \"string\"\n        },\n        \"expireWhenRemediationAvailable\": {\n          \"description\": \"If true, the waiver will automatically expire when a remediation becomes available. Can only be set to true when matcherStrategy is EXACT_COMPONENT.\",\n          \"example\": false,\n          \"type\": \"boolean\"\n        },\n        \"expiryTime\": {\n          \"description\": \"Optional expiration date/time for the waiver in ISO 8601 format. Must be in the future if provided.\",\n          \"format\": \"date-time\",\n          \"type\": \"string\"\n        },\n        \"matcherStrategy\": {\n          \"description\": \"Component matching strategy. For Firewall bulk waivers, only EXACT_COMPONENT and ALL_VERSIONS are supported.\",\n          \"enum\": [\n            \"DEFAULT\",\n            \"EXACT_COMPONENT\",\n            \"ALL_COMPONENTS\",\n            \"ALL_VERSIONS\",\n            \"EXACT_COMPONENT\",\n            \"ALL_VERSIONS\"\n          ],\n          \"example\": \"EXACT_COMPONENT\",\n          \"type\": \"string\"\n        },\n        \"waiverReasonId\": {\n          \"description\": \"Optional reference to a pre-defined waiver reason ID\",\n          \"example\": \"waiver-reason-id-123\",\n          \"type\": \"string\"\n        }\n      },\n      \"required\": [\n        \"comment\",\n        \"matcherStrategy\"\n      ],\n      \"type\": \"object\"\n    },\n    \"ownerId\": {\n      \"description\": \"Enter the id for the " + "\x60" + "ownerType" + "\x60" + " provided above. E.g. " + "\x60" + "applicationId" + "\x60" + " if the " + "\x60" + "ownerType" + "\x60" + " is application.\",\n      \"type\": \"string\"\n    },\n    \"ownerType\": {\n      \"description\": \"Indicates the scope of the policy waiver. Possible values are application, organization, repository, repository_manager, and repository_container.\",\n      \"enum\": [\n        \"application\",\n        \"organization\",\n        \"repository_container\",\n        \"repository_manager\",\n        \"repository\"\n      ],\n      \"pattern\": \"application|organization|repository|repository_manager|repository_container\",\n      \"type\": \"string\"\n    },\n    \"policyWaiverId\": {\n      \"description\": \"Enter the id for the policy waiver.\",\n      \"type\": \"string\"\n    }\n  },\n  \"required\": [\n    \"body\",\n    \"ownerId\",\n    \"ownerType\",\n    \"policyWaiverId\"\n  ],\n  \"type\": \"object\"\n}"

// NewUpdatePolicyWaiverMCPTool creates the MCP Tool instance for UpdatePolicyWaiver
func NewUpdatePolicyWaiverMCPTool() mcp.Tool {
	return mcp.NewToolWithRawSchema(
		"UpdatePolicyWaiver",
		"Use this method to update an existing policy waiver.\n\nPermissions required: Waive Policy Violations",
		[]byte(UpdatePolicyWaiverInputSchema),
	)
}

// UpdatePolicyWaiverHandler is the handler function for the UpdatePolicyWaiver tool.
// It reads tool arguments, forwards the request to the upstream service, and returns the response.
func UpdatePolicyWaiverHandler(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	upstream := mcputils.GetUpstreamEndpoint()

	args := request.GetArguments()
	if args == nil {
		args = make(map[string]interface{})
	}
	contentType := ""
	return mcputils.ForwardAndParseResponse(ctx, upstream, "PUT", "/api/v2/policyWaivers/{ownerType}/{ownerId}/{policyWaiverId}", args, []string{"ownerId", "ownerType", "policyWaiverId"}, contentType, "UpdatePolicyWaiver")
}
