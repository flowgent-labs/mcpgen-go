...
description: 根据开发者描述的业务场景，自动生成符合 mcpgen virtual tools 规范的聚合工具配置 (YAML)。
---

# Virtual Tool Creator

根据开发者的自然语言描述或现有脚本（bash/jq/yq），生成符合 [dsl-schema.json](resources/dsl-schema.json) 规范的 `virtualTools` YAML 配置。
...
| 资源 | 说明 |
|------|------|
| [dsl-schema.json](resources/dsl-schema.json) | **权威结构定义** — 由 `cmd/virtual-tool-dsl-schema-gen/main.go` 从 Go struct 生成 |
